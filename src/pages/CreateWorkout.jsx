...
import { Plus, Trash2, Save, X, Check, ChevronRight, Timer, Dumbbell, Flag, FlagOff } from 'lucide-react'
...
function ExercisePicker({ onAdd, onClose, existingNames = [] }) {
  ...
  const handleConfirm = () => {
    ...
    onAdd({
      id: Math.random(),
      name: selected,
      meters: isErgo(selected) ? meters : '',
      reps: !isErgo(selected) ? reps : '',
      notes
    })
    onClose()
  }
...
// ─── BLOCCO ESERCIZIO ─────────────────────────────────────────
function ExerciseRow({ ex, index, onRemove, showMinute }) {
  return (
    <div className="flex items-center gap-3 bg-[#222] border border-[#2e2e2e] rounded-2xl px-4 py-3">
      {showMinute && (
      ...
      <button onClick={() => onRemove(ex.id)} className="text-gray-700 hover:text-red-400 transition">
        <Trash2 size={15} />
      </button>
    </div>
  )
...
// ─── BLOCCO CASH IN/OUT ───────────────────────────────────────
function CashBlock({ label, exercises, onAdd, onRemove, icon }) {
    ...
    <div className="bg-[#1e1e1e] border border-[#2e2e2e] rounded-2xl p-4">
      ...
      {exercises.length === 0 ? (
        ...
        <div className="flex flex-col gap-2">
          {exercises.map((ex, i) => (
            <ExerciseRow key={ex.id} ex={ex} index={i} onRemove={onRemove} showMinute={false} />
          ))}
        </div>
      )}
      {pickerOpen && (
...
export default function CreateWorkout() {
  ...
  const handleSave = async () => {
    ...
    const { data: newWorkout, error } = await supabase.from('workouts').insert({
      title, date, sections, coach_notes: coachNotes
    }).select().single()
  ...
  return (
    <div className="p-4 max-w-2xl mx-auto pb-24">
      <h1 className="text-2xl font-bold text-[#f1ba17] mb-6">Crea Workout</h1>
