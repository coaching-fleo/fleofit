...
| 15-bis | `React.memo` su `RunningStepRow` | ~2 ore | Stessa tecnica, stesso guadagno minore (le fasi di corsa non hanno scroll picker da 102 opzioni). Il contratto resta asimmetrico: `RunningStepRow` passa l'indice, non l'id |
...
| 19 | Test sulle pagine: **CreateWorkout si monta**, le altre no | grande | ✅ Il blocco vero non erano i finti `supabase` e router: era che **jsdom espone un `localStorage` rotto** (`getItem is not a function`), e ogni pagina lo legge in un effetto. Risolto il 26/08 con un `localStorage` in memoria in `src/test/setup.js`. Da lì `CreateWorkout` si monta e si pilota davvero: 5 test ci girano sopra. Restano Home, WorkoutDetail e AthleteDetail, che hanno bisogno anche di AuthContext e di Supabase molto più finto |
