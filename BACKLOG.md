...
| 19 | Test sulle pagine: 2 componenti su ~30 | grande | ✅ Infrastruttura montata il 25/08 (jsdom + testing-library, `vitest.config.js`) e 37 test su `HyroxBlock` e `RunningStepRow`, i due che la voce 15 vuole toccare. **Restano scoperte le pagine intere**: Home, WorkoutDetail, AthleteDetail e CreateWorkout non si montano senza finti `supabase`, router e AuthContext. Il candidato successivo è la **coda offline** di Home, dove un guasto costa dati (§9-quater) |
